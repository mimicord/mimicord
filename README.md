...
このリポジトリには汎用実装とサンプル設定のみを含めます。作者インスタンス固有の人格 config・デプロイ設定・シークレットは別の private リポジトリで管理します（Misskey 本体とインスタンス運用の分離と同じ構図）。
...
3. **稼働の安定性** — 暴走・連投をしない。LLM コストが予算（月額 3 万円）内に収まり、無人で回り続ける。
...
- **ハードリミット**: 発話レート（連投防止）と LLM コスト（月額予算 3 万円）に上限を設け、超過時は自動で発話を止める。
