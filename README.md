...
このリポジトリには汎用実装とサンプル設定のみを含めます。作者インスタンス固有の人格 config・デプロイ設定・シークレット・予算などの運用パラメータは別の private リポジトリで管理します（Misskey 本体とインスタンス運用の分離と同じ構図）。
...
ライセンスは未定です（upstream-of-service モデルでの選定を検討中）。決定までコードへの外部コントリビューションは受け付けられませんが、設計についての議論・フィードバックは Issue / Discussion で歓迎します。
